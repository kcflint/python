...
class Cell:
    ...
    def __add__(self, other):
        return Cell("Объединение двух клеток", self.number + other.number)

    def __sub__(self, other):
        if self.number - other.number > 0:
            return Cell("Вычитание", self.number - other.number)
        else:
            print("Разница меньше нуля")

    def __mul__(self, other):
        return Cell("Умножение", self.number * other.number)

    def __truediv__(self, other):
        return Cell("Целочисленное деление", self.number // other.number)

    def __str__(self):
        return f"Клетка: {self.name}. Количество ячеек {self.number}"

    def make_order(self, line):
        return ("*" * line + "|" + "n") * (self.number // line) + "*" * (self.number % line)
